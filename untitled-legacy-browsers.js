...
flowScheduler.add(zfdfcvRoutineBegin());
flowScheduler.add(zfdfcvRoutineEachFrame());
flowScheduler.add(zfdfcvRoutineEnd());
...
async function experimentInit() {
  ...
  // Initialize components for Routine "zfdfcv"
  zfdfcvClock = new util.Clock();
...
function one_repeat_loopLoopBegin(one_repeat_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    one_repeat_loop.forEach(function() {
...
function expritaRoutineBegin(snapshot) {
  return async function () {
    ...
    expritaComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function let_2RoutineBegin(snapshot) {
    return async function () {
      ...
      let_2Components.forEach( function(thisComponent) {
        if ('status' in thisComponent)
          thisComponent.status = PsychoJS.Status.NOT_STARTED;
         });
      return Scheduler.Event.NEXT;
...
function answrita1RoutineBegin(snapshot) {
    return async function () {
      ...
      answrita1Components.forEach( function(thisComponent) {
        if ('status' in thisComponent)
          thisComponent.status = PsychoJS.Status.NOT_STARTED;
         });
      return Scheduler.Event.NEXT;
...
function rtertgeRoutineBegin(snapshot) {
    return async function () {
      ...
      body: JSON.stringify({ 
      experimentID: '0ROnV3xEPV9c', // * обновить, указав experiment ID из DATAPIPE на шаге 4.3 * 
      filename: filename,  
      data: data, 
      }), 
      ...
      rtertgeComponents.forEach( function(thisComponent) {
        if ('status' in thisComponent)
          thisComponent.status = PsychoJS.Status.NOT_STARTED;
         });
      return Scheduler.Event.NEXT;
...
var zfdfcvMaxDurationReached;
var zfdfcvMaxDuration;
var zfdfcvComponents;
function zfdfcvRoutineBegin(snapshot) {
    return async function () {
      TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
      
      //--- Prepare to start Routine 'zfdfcv' ---
      t = 0;
      frameN = -1;
      continueRoutine = true; // until we're told otherwise
      // keep track of whether this Routine was forcibly ended
      routineForceEnded = false;
      zfdfcvClock.reset(routineTimer.getTime());
      routineTimer.add(1.000000);
      zfdfcvMaxDurationReached = false;
      // update component parameters for each repeat
      // Disable downloading results to browser 
      psychoJS._saveResults = 0; 
      // Generate filename for results 
      ...
      body: JSON.stringify({ 
      experimentID: '2m6DUAD9C0DJ', // * обновить, указав experiment ID из DATAPIPE на шаге 4.3 * 
      filename: filename,  
      data: data, 
      }), 
      }).then(response => response.json()).then(data => { 
      ...
      psychoJS.experiment.addData('zfdfcv.started', globalClock.getTime());
      zfdfcvMaxDuration = null
      // keep track of which components have finished
      zfdfcvComponents = [];
      zfdfcvComponents.push(text_4);
      
      zfdfcvComponents.forEach( function(thisComponent) {
        if ('status' in thisComponent)
          thisComponent.status = PsychoJS.Status.NOT_STARTED;
         });
      return Scheduler.Event.NEXT;
...
function zfdfcvRoutineEachFrame() {
    return async function () {
      //--- Loop for each frame of Routine 'zfdfcv' ---
      // get current time
      t = zfdfcvClock.getTime();
      frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
      // update/draw components on each frame
...
function zfdfcvRoutineEnd(snapshot) {
    return async function () {
      //--- Ending Routine 'zfdfcv' ---
      zfdfcvComponents.forEach( function(thisComponent) {
        if (typeof thisComponent.setAutoDraw === 'function') {
          thisComponent.setAutoDraw(false);
        }
      });
      psychoJS.experiment.addData('zfdfcv.stopped', globalClock.getTime());
      if (routineForceEnded) {
          routineTimer.reset();} else if (zfdfcvMaxDurationReached) {
          zfdfcvClock.add(zfdfcvMaxDuration);
      } else {
          zfdfcvClock.add(1.000000);
      }
      // Routines running outside a loop should always advance the datafile row
      if (currentLoop === psychoJS.experiment) {
        psychoJS.experiment.nextEntry(snapshot);
